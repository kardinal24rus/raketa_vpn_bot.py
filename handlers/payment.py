...
async def buy_stars(callback: CallbackQuery):
    ...
    prices = [
        LabeledPrice(
            ...
            amount=pack["stars"]  # Для Stars можно оставить как есть
        )
    ]
    ...
    await callback.message.answer_invoice(
        ...
        provider_token="",  # Для теста Stars можно оставить пустым
        currency="XTR",
        prices=prices
    )
    await callback.answer()
...
async def successful_payment(message: Message, state: FSMContext):
    ...
    if payload.startswith("stars:"):
        ...
        # Автоматически возвращаем пользователя в профиль после оплаты
        from main import show_start_content  # Импорт функции из основного кода
