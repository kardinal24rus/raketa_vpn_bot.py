    ...
    if "language" not in data:
        ...
        await message.answer(
            ...
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[
                    [InlineKeyboardButton(text=languages_flags[0][0], callback_data=f"lang_{languages_flags[0][1]}"),
                     InlineKeyboardButton(text=languages_flags[1][0], callback_data=f"lang_{languages_flags[1][1]}")]
                ]
            )
        )
    else:
...
async def callback_handler(callback: CallbackQuery, state: FSMContext):
    ...
    if data.startswith("input_"):
        ...
        await callback.message.answer(t["input_prompt"].format(field=t.get(field, field)),
