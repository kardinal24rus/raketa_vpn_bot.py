...
async def show_main_menu(message, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    keyboard = [
        [KeyboardButton(texts['btn_partial_search'])],
        [KeyboardButton(texts['btn_profile']), KeyboardButton(texts['btn_bots'])],
        [KeyboardButton(texts['btn_partner'])]
    ]
...
async def handle_menu_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if text == texts['btn_partial_search']:
        # Начать поиск
        await start_search(update, context)
    
    elif text == texts['btn_profile']:
        # Мой профиль
        user = update.effective_user
        profile_text = {
            'ru': f"👤 Ваш профиль:\n\n"
        ...
        await update.message.reply_text(profile_text[lang])
    
    elif text == texts['btn_bots']:
        # Мои боты
        bots_text = {
            'ru': "🤖 Мои боты:\n\nЗдесь будет список ваших ботов.",
            'en': "🤖 My Bots:\n\nYour bots list will be here."
        }
        await update.message.reply_text(bots_text[lang])
    
    elif text == texts['btn_partner']:
        # Партнерская программа
        partner_text = {
            'ru': "💰 Партнерская программа:\n\nПриглашайте друзей и получайте бонусы!",
            'en': "💰 Partner Program:\n\nInvite friends and get bonuses!"
        }
        await update.message.reply_text(partner_text[lang])
...
def main() -> None:
    ...
    search_conv = ConversationHandler(
        entry_points=[
            ...
            MessageHandler(filters.Regex('^🔍'), start_search),
        ],
        states={
            WAITING_NAME: [
